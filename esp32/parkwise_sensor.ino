/*
 * ============================================================
 *  ParkWise — ESP32 Parking Sensor Firmware
 * ============================================================
 *
 *  Hardware:
 *    - ESP32 Dev Board (30-pin)
 *    - 4× HC-SR04  → parking slot occupancy (A1–A4)
 *    - 1× HC-SR04  → entrance gate car detection
 *    - 1× Servo    → gate barrier (GPIO 13)
 *    - 1× 16×2 I2C LCD (SDA=21, SCL=22)
 *    - 9V battery  → via buck converter to 5V recommended
 *
 *  Behavior:
 *    1. Slot sensors: distance < 5cm → occupied
 *    2. Entrance sensor: distance < 7cm → car detected →
 *       servo opens gate → count++ → close after 3s
 *    3. LCD shows available slots + cars entered
 *    4. Sends JSON to Node.js server via WiFi every 1s
 *
 * ============================================================
 */
...
void loop() {
  ...
  for (int i = 0; i < NUM_SLOTS; i++) {
    ...
    Serial.printf("  %s: %.1f cm → %s\n",
      SLOT_IDS[i], dist, slotOccupied[i] ? "OCCUPIED" : "VACANT");
  }
  ...
  bool carAtEntrance = (entranceDist > 0 && entranceDist < ENTRANCE_THRESHOLD_CM);
  unsigned long now = millis();

  if (carAtEntrance && !gateOpen && (now - lastEntranceTrigger > ENTRANCE_COOLDOWN_MS)) {
